...
boru-driven command-line client **and** interactive REPL for the YuGiOh
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yu-gi-oh-cli)
make build
...
YU_GI_OH_BASE=https://api.example.com ./yu-gi-oh-cli list cardinfo

# 6. No arguments -> interactive REPL
./yu-gi-oh-cli
yu-gi-oh> list cardinfo
yu-gi-oh> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yu-gi-oh-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/yu-gi-oh-cli list cardinfo
   ```
...
Run with no arguments to open a REPL (prompt `yu-gi-oh>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `cardinfo`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/yu-gi-oh-cli`. |
...
(`./yu-gi-oh-cli <expr>`) and interactively (the REPL), and expressions compose the
