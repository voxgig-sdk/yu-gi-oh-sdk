...
The Lua SDK for the YuGiOh API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Cardinfo(nil):list(nil, nil)
...
local result, err = client:YuGiOh(nil):load(
  { id = "test01" }, nil
)
