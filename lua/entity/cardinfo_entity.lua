...
local CardinfoEntity = {}
CardinfoEntity.__index = CardinfoEntity


function CardinfoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CardinfoEntity)
  self._name = "cardinfo"
  self._client = client
...
function CardinfoEntity:get_name()
  return self._name
end


function CardinfoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CardinfoEntity.new(self._client, opts)
end
...
function CardinfoEntity:mark_deleted()
  self._deleted = true
end


function CardinfoEntity:deleted()
  return true == self._deleted
end


function CardinfoEntity:data_set(args)
  if args ~= nil then
...
function CardinfoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CardinfoEntity:match_set(args)
  if args ~= nil then
...
function CardinfoEntity:match_get()
...
function CardinfoEntity:stream(action, args, callopts)
...
---@param reqmatch CardinfoListMatch
---@param ctrl? table
---@return Cardinfo[]
---@return string? err
function CardinfoEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CardinfoEntity:_run_op(ctx, post_done)
  local utility = self._utility
