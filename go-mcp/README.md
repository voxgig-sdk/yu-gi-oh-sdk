...
[MCP](https://modelcontextprotocol.io) server exposing the YuGiOh SDK as
two agent tools — `yu-gi-oh_list` and `yu-gi-oh_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/yu-gi-oh-mcp)
make build

# 2. Provide credentials via the environment
export YU_GI_OH_APIKEY=sk_live_xxx
...
```jsonc
// yu-gi-oh_list: first page of records
{ "entity": "cardinfo" }
{ "entity": "cardinfo", "query": { } }

// yu-gi-oh_load: one record by id
{ "entity": "cardinfo", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/yu-gi-oh-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user yu-gi-oh \
     -- "$PWD"/dist/*/yu-gi-oh-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `yu-gi-oh_list` and `yu-gi-oh_load` tools now appear
   in new sessions. Ask the agent to *"list cardinfo using yu-gi-oh"*
   and it calls `yu-gi-oh_list` with `{"entity":"cardinfo"}`.
...
```sh
export YU_GI_OH_APIKEY=sk_live_xxx            # API key
export YU_GI_OH_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `yu-gi-oh_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `yu-gi-oh_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `YU_GI_OH_APIKEY` | API key sent with every request. |
| `YU_GI_OH_BASE` | Optional override of the API base URL. |
