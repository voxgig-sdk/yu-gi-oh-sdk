...
//    only - a typed YuGiOhContext there would make every ctx
...
final class YuGiOhOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new YuGiOhContext($clean, null);
    ...
    public static function err_from_map($m): ?YuGiOhError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new YuGiOhError(is_string($code) ? $code : '', $msg);
    }
