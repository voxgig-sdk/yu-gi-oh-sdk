...
class FtCtrl extends YuGiOhControl
{
...
class FtClient
{
    ...
    private YuGiOhUtility $utility;

    public function __construct(YuGiOhUtility $utility, array $options)
    {
    ...
    public function get_utility(): YuGiOhUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (YuGiOhContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new YuGiOhUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (YuGiOhContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = YuGiOhConfig::make_feature($name);
    ...
    private static function buildUrl(YuGiOhSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof YuGiOhError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new YuGiOhResponse($fetched);
        }
    ...
    private function fail(YuGiOhContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (YuGiOhContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
