...
The Python SDK for the YuGiOh API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Cardinfo(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.YuGiOh(None).load(
    {"id": "test01"}, None
)
