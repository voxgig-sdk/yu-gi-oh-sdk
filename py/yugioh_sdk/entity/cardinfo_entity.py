...
from yugioh_sdk.yugioh_types import (
    Cardinfo,
    CardinfoListMatch,
)
...
class CardinfoEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CardinfoEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Cardinfo]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Cardinfo().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
