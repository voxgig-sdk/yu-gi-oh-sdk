...
The TypeScript SDK for the YuGiOh API. Provides a type-safe, entity-oriented interface with full async/await support.
...
```ts
const result = await client.Cardinfo().list()
...
```ts
const client = new YuGiOhSDK({ apikey: '...' })
