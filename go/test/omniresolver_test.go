...
import (
	...
	omni "github.com/voxgig-sdk/yu-gi-oh-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.YuGiOhError); is {
				out := map[string]any{
					"name":    "YuGiOhError",
...
func omniCtx(arg any, client *sdk.YuGiOhSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.YuGiOhSDK); is {
			client = live
