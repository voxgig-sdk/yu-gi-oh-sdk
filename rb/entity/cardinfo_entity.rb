...
class CardinfoEntity
  ...
  def make
    opts = @_entopts.dup
    CardinfoEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = YuGiOhHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = YuGiOhHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [CardinfoListMatch, Hash, nil] match filter (any subset of
  #   Cardinfo fields); defaults to nil, treated as an empty match that lists all.
