...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = YuGiOhOmni.make_runner(TEST_JSON_FILE, YuGiOhSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : YuGiOhOmni.errify(err)
    end
