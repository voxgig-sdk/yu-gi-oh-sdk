...
class CardinfoEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = YuGiOhSDK.test(seed, nil)
    seen = base.Cardinfo(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Cardinfo(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set YU_GI_OH_TEST_CARDINFO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    cardinfo_ref01_list_result = cardinfo_ref01_ent.list(cardinfo_ref01_match, nil)
...
def cardinfo_basic_setup(extra)
  ...
  entid_env_raw = ENV["YU_GI_OH_TEST_CARDINFO_ENTID"]
