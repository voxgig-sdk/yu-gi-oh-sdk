...
class CardinfoDirectTest < Minitest::Test
  def test_direct_list_cardinfo
    setup = cardinfo_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-cardinfo", setup[:live] ? "live" : "unit")
    if _should_skip
...
def cardinfo_direct_setup(mockres)
