...
class YuGiOhSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = YuGiOhHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = YuGiOhHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!YuGiOhFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, YuGiOhFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return YuGiOhUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = YuGiOhHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = YuGiOhHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = YuGiOhHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new YuGiOhError($op . "_allow",
                "YuGiOhSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = YuGiOhHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new YuGiOhError("graphql_error",
                "YuGiOhSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Cardinfo($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_cardinfo === null) {
                $this->_cardinfo = new CardinfoEntity($this, null);
            }
            return $this->_cardinfo;
        }
        return new CardinfoEntity($this, $data);
    }
