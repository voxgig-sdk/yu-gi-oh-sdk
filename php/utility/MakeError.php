...
class YuGiOhMakeError
{
    public static function call(?YuGiOhContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new YuGiOhContext([], null);
        }
        $op = $ctx->op ?? new YuGiOhOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new YuGiOhResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof YuGiOhError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new YuGiOhError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof YuGiOhError) {
