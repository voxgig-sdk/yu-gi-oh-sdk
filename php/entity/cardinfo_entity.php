...
class CardinfoEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CardinfoEntity($this->_client, $opts);
    }

    /**
     * @param Cardinfo|array $args Cardinfo data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = YuGiOhHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Cardinfo|array The current Cardinfo data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Cardinfo fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = YuGiOhHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Cardinfo fields).
     */
     ...
     *
     * @param CardinfoListMatch|array|null $reqmatch Match filter (any subset
     *   of Cardinfo fields) as an assoc-array; CardinfoListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Cardinfo[]|array A list of Cardinfo items as assoc-arrays at
     *   the SDK boundary; throws YuGiOhError on failure (item-5 convention).
     */
